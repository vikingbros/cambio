...
class Player:
    ...
    def show(self, position: int) -> Optional[Card]:
        try:
            print(self.cards[position])
            return self.cards[position]
        except IndexError:
            print(
                f"Positon provided is out of bounds, max position possible is: {len(self.cards) - 1}"
            )
            return None

    def replace(
        self, position: int, card: Optional[Card], deck: Optional[Deck] = None
    ) -> None:
    ...
    def stack(self, position: int, deck: Deck) -> None:
        if deck.discard[-1].value == self.cards[position].value:
            ...
            self.cards.append(deck.draw()[0])

    def stack_other_player(
        self, other_player_position: int, other_player: "Player", deck: Deck
    ) -> bool:
